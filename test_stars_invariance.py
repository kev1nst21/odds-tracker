...
# --- 12. острая контора не может в одиночку открыть публикацию --------------
# 21.08.2026. 18.08 лазейку `or sharp_down` убрали из well_evidenced и
# посчитали вопрос закрытым. Убрали половину: бонус за острую контору
# применялся ПОСЛЕ потолка по счёту, поэтому движение у ДВУХ контор получало
# 1★ по счёту, +1 за остроту и уходило в эфир как ★★.
#
# Так в книгу попал «Felixstowe and Walton United» — 2 конторы из 10 в
# предварительном раунде Кубка Англии. Два букмекера это не рынок, а два
# букмекера, и никакая острота одного из них этого не меняет.
for moved in range(1, config.MOVED_FOR_2_STARS):
    books = {f"b{i}" for i in range(moved)}
    with_sharp = analytics._stars(books, True, 50)
    assert with_sharp < config.MIN_SIGNAL_STARS, (
        f"{moved} контор + острая дали {with_sharp}★ — это публикация, "
        "открытая одной конторой")
print(f"claim ok: меньше {config.MOVED_FOR_2_STARS} контор не публикуется даже "
      "с острой конторой среди них")

# Но там, где счёт и так прошёл, острота по-прежнему усиливает — иначе мы бы
# выбросили полезный признак вместе с лазейкой.
qual = {f"b{i}" for i in range(config.MOVED_FOR_2_STARS)}
assert analytics._stars(qual, True, 50) > analytics._stars(qual, False, 50), (
    "бонус за острую контору перестал работать там, где он законен")
print("claim ok: на прошедшем порог счёте острая контора по-прежнему добавляет ступень")

# --- 13. порог глубины рынка отсекает то, что рынком не является -----------
# «Двинулось 3 из 4» — это 75% и это три букмекера. Порог глубины существует
# ровно для того, чтобы доля не считалась от чисел, у которых нет знаменателя.
assert config.MIN_MARKET_BOOKS >= 5, (
    f"порог глубины опущен до {config.MIN_MARKET_BOOKS} — при таком значении "
    "в книгу снова попадут рынки из четырёх контор, где доля считается от "
    "числа без знаменателя")
print(f"claim ok: рынок тоньше {config.MIN_MARKET_BOOKS} контор рынком не считается")
