...
# Поднято с 3 до 8 — 21.08.2026.
#
# Причина в замере, а не в осторожности. За 21.08 воронка показала: из десяти
# найденных падений ШЕСТЬ отсеялось как тонкий рынок — то есть отсев по
# глубине стал главным, обогнав и звёзды, и цену входа. Одновременно медиана
# глубины у опубликованных событий рухнула с 47-54 контор до 11: закончился
# Цинциннати, который держал теннис на пятидесяти конторах, а расширение до
# 126 лиг за цикл притащило предварительные раунды Кубка Англии и саудовскую
# лигу, которую котируют ЧЕТЫРЕ конторы.
#
# «Двинулось 3 из 4» — это не 75% рынка, это три букмекера. Порог в три
# конторы позволял такому публиковаться и тратил на это наш же капитал
# доверия. По книге из 28 сигналов подъём до 12 убирает ровно три: Al-Qadsiah
# (4 конторы), Liversedge (11) и Felixstowe (10) — все три из тех, что
# появились в последние сутки и все три с рынком, которого нет на Polymarket.
# СНАЧАЛА ПОСТАВИЛ 12 И ОТКАТИЛ. Двенадцать убирало три сигнала из 28, но
# заодно выносило всю программу малых лиг, ради которой 01.08 порог и опускали
# до трёх: тогда требование «минимум 4 конторы» отсеяло 6 падений из 6, и
# широкий обход не давал вообще ничего. То решение было правильным для своего
# времени, и ломать его мимоходом нельзя.
#
# Восемь — это граница между «рынок» и «несколько букмекеров». По книге из 28
# сигналов она убирает ровно один: Al-Qadsiah, 3 конторы из ЧЕТЫРЁХ. Малые
# лиги с десятью-одиннадцатью конторами остаются в игре, а тонкие движения в
# них по-прежнему отсеиваются лестницей звёзд, а не запретом на лигу целиком.
MIN_MARKET_BOOKS = int(os.getenv("MIN_MARKET_BOOKS", "8"))
...
# POLYMARKET
# ---------------------------------------------------------------------------
# Добавлено 20.08.2026. Владислав: «основной инструмент через который я планирую
# работать — это полимаркет... задача отслеживать события на которые полимаркет
# даёт коэффициент выше чем наши конторы, хотя бы на 5%».
#
# База сравнения выбрана им же и однозначно: ЛУЧШАЯ ЦЕНА ВХОДА В БК по этому
# сигналу, не справедливая цена и не витрина Polymarket. Порог сверху — 5%.
POLYMARKET_ENABLED = os.getenv("POLYMARKET_ENABLED", "1") not in ("0", "false", "False")
# 20.08.2026, вечером того же дня: порог опущен с 5% до НУЛЯ. Дословно —
# «зазор только не 5%, а 0%, то есть если от 0 и выше. Главное не хуже».
#
# Это не смягчение ради объёма, у этого есть содержательная причина. Пять
# процентов были платой за риск, которого на ордербуке нет: у букмекера
# выигрышный счёт режут лимитами и закрывают, поэтому там имеет смысл ходить
# только за заметным перевесом. На Polymarket видна глубина, нет банов и нет
# проблем с выводом — поэтому цена РОВНО ТАКАЯ ЖЕ, как у конторы, там уже
# лучше просто потому, что её можно взять в размере и повторить завтра.
#
# ЧЕГО ЭТО СТОИТ, И ЭТО НАДО ЗНАТЬ. Пятипроцентный порог заодно молча
# проглатывал любые издержки — комиссию, газ, конвертацию. На нуле этой
# подушки больше нет: если реальные издержки окажутся хоть сколько-нибудь
# отличными от нуля, сделка «ровно вровень» станет минусовой. Поэтому
# POLYMARKET_COST_FRAC ниже остаётся ручкой, и первый же замер фактических
# издержек на живой сделке должен быть в неё вписан. До тех пор ноль здесь —
# осознанное допущение, а не установленный факт.
POLYMARKET_MIN_EDGE_PCT = float(os.getenv("POLYMARKET_MIN_EDGE_PCT", "0"))

# Размер, на который мы хотим влезть. Если стакан по нужной цене держит меньше,
# в журнал пишется фактически влезающая сумма, а не отказ — прямая просьба:
# «если к примеру не залезаем, то писать ту сумму в таблице по которой залезли».
POLYMARKET_TARGET_STAKE = float(os.getenv("POLYMARKET_TARGET_STAKE", "200"))

# Издержки сверх порога. Ноль намеренно: пять процентов и есть запас, и второй
# слой означал бы считать его дважды. Ручка ждёт ФАКТИЧЕСКОГО замера комиссии
# CLOB, газа Polygon и конвертации USDC на реальной сделке — до тех пор здесь
# честный ноль, а не выдуманное число.
POLYMARKET_COST_FRAC = float(os.getenv("POLYMARKET_COST_FRAC", "0"))

# Теги индекса. Оба замерены живьём 20.08: 1200 открытых футбольных событий и
# 127 теннисных.
POLYMARKET_TAGS = os.getenv("POLYMARKET_TAGS", "soccer,tennis")

# Индекс переиспользуется между сигналами внутри цикла: он общий для всех и
# весит один и тот же обход. Запросы бесплатные, ограничение здесь — время.
POLYMARKET_INDEX_TTL_MINUTES = float(os.getenv("POLYMARKET_INDEX_TTL_MINUTES", "20"))

# Окно сверки даты. Polymarket проставляет событию то время листинга, то время
# матча, и расхождение в сутки — норма, а не ошибка. Шире делать нельзя:
# начнём хватать соседний тур тех же команд.
POLYMARKET_DATE_WINDOW_HOURS = float(os.getenv("POLYMARKET_DATE_WINDOW_HOURS", "36"))

# Звёзды Polymarket — отдельная шкала, не копия букмекерской.
#
# Просьба 20.08: «придумывай правила условия по полику, такие же звезды и т.д.».
# Копировать нашу лестницу сюда было бы ошибкой: она меряет СИЛУ ДВИЖЕНИЯ (у
# скольких контор поехала линия), а на Polymarket решает не это, а размер
# зазора и глубина под него. Сильный сигнал с зазором в 5% и стаканом на $30 —
# это не то же самое, что средний сигнал с зазором 12% и полным размером.
#
# Поэтому здесь три составляющие, и все три обязательны для верхних ступеней:
#   1) насколько Polymarket лучше конторы (сам зазор);
#   2) влезает ли туда полный размер (зазор на $20 — это не сделка);
#   3) что говорил наш собственный сигнал (шкала контор как множитель доверия).
# ОТСТАВАНИЕ POLYMARKET — доля движения, которую он ещё не отыграл.
#   1.0 — стоит на цене ДО движения, не шелохнулся;
#   0.0 — отыграл всё, стоит там же, где конторы после движения.
# Верхняя ступень требует, чтобы он не отыграл почти ничего.
PM_LAG_4_STARS = float(os.getenv("PM_LAG_4_STARS", "0.8"))
PM_LAG_3_STARS = float(os.getenv("PM_LAG_3_STARS", "0.5"))

# Пороги ширины берутся ОТТУДА ЖЕ, откуда их берёт основная шкала
# (MOVED_FOR_3_STARS / MOVED_FOR_4_STARS выше). Две шкалы меряют разное, но
# вопрос «настоящее ли это движение» у них общий, и разводить на него два
# набора чисел значило бы однажды поправить один и забыть другой.


# --- Polymarket: сколько стаканов трогаем за цикл --------------------------
# 21.08.2026, вместе с разворотом воронки. Сопоставление названий локальное и
# бесплатное, поэтому со списком событий мы сверяем ВСЁ, что котируем. Платим
# временем только за стакан, и вот его и ограничиваем.
#
# Шестьдесят запросов на цикл при пятиминутном шаге -- это примерно один
# запрос в пять секунд, что для публичного API вежливо и при этом даёт 17
# тысяч заглядываний в стакан за сутки против прежних трёхсот шестидесяти.
PM_MAX_BOOKS_PER_CYCLE = int(os.getenv("PM_MAX_BOOKS_PER_CYCLE", "60"))

# Сколько пар из общего котируемого множества тянуть на сопоставление.
PM_UNIVERSE_LIMIT = int(os.getenv("PM_UNIVERSE_LIMIT", "1200"))
# Пара, которую держат меньше этого числа контор, для сравнения не годится:
# «лучшая цена» у трёх букмекеров -- это не рынок, а случайность.
# Опущено с 6 до 4 — 21.08, по просьбе «может снизить требования от
# количества контор». Здесь это безопаснее, чем в сигналах: для публикации
# сигнала глубина рынка — вопрос доверия к движению, а тут движение вообще ни
# при чём. Нужна одна вещь: чтобы «лучшая цена у контор» что-то значила.
# Четыре независимых котировки этому условию удовлетворяют, а сравнение всё
# равно упирается в стакан Polymarket, а не в наш знаменатель.
PM_UNIVERSE_MIN_BOOKS = int(os.getenv("PM_UNIVERSE_MIN_BOOKS", "4"))
