...
def _stars(down_books: set, sharp_moved: bool, books_count: int = 0) -> int:
    ...
    if books_count:
        ...
        stars = by_count
    if sharp_moved:
        # БОНУС ЗА ОСТРУЮ КОНТОРУ -- МОДИФИКАТОР КАЧЕСТВА, А НЕ ПРОПУСК В ЭФИР.
        #
        # 21.08.2026. 18.08 я убрал лазейку `or sharp_down` из well_evidenced,
        # решив, что одна острая контора больше не может открыть публикацию в
        # одиночку. Оказалось, что убрал только половину: бонус применялся
        # ПОСЛЕ потолка по счёту, поэтому движение у ДВУХ контор получало
        # 1★ по счёту, +1 за острую и уходило в эфир как ★★.
        #
        # Так в книгу попал Felixstowe and Walton United -- 2 конторы из 10 в
        # предварительном раунде Кубка Англии. Два букмекера -- это не рынок,
        # это два букмекера, и назвать это сигналом нельзя ни при какой
        # остроте одного из них.
        #
        # Теперь бонус не может сам по себе поднять событие до порога
        # публикации: счёт обязан независимо дотянуть до нижней ступени.
        # Острая контора по-прежнему усиливает то, что и так прошло.
        base_qualifies = n >= MOVED_FOR_2_STARS
        bumped = min(MAX_STARS, stars + 1)
        stars = bumped if (base_qualifies or bumped < MIN_SIGNAL_STARS) else stars
    return stars
